...
class GameProvider extends ChangeNotifier {
  ...
  void gameOverListener({
    ...
    if (game.gameOver) {
      ...
      // cancel the gameStream subscription
      ...
      if (availableGames.docs.isNotEmpty) {
        ...
        if (gamesList.isEmpty) {
          ...
          createNewGameInFireStore(
              userModel: userModel, onSuccess: onSuccess, onFail: onFail);
        } else {
        ...
        _waitingText = Constants.searchingPlayerText;
        notifyListeners();
        // we do not have any available games - create a game
        createNewGameInFireStore(
            userModel: userModel, onSuccess: onSuccess, onFail: onFail);
      }
...
// create a game
  ...
  void joinGame({
    ...
    try {
      ...
      if (myGame.exists) {
        // delete our created game since we are joining another game
    ...
    gameStreamSubscription =
      ...
      if (event.docs.isNotEmpty) {
        ...
        if (game[Constants.gameCreatorUid] == userModel.uid) {
          // check if it's white' turn
          if (game[Constants.isWhitesTurn]) {
            _isWhitesTurn = true;

            // check if blackCurrentMove is not empty or equal the old move - means black has played his move
          ...
          // check if white played his move
